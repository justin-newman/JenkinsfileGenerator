        ...
        public override string DeployTo(string solutionName, string pipelineName, string projectName, string deployToLocation, string OctopusProjectName, string FrameworkVersion)
        {
            ...
            StringBuilder sb = new StringBuilder();
                ...
                sb.Append($"		    stage ('Deploy {OctopusProjectName} to {deployToLocation}') {{\r\n" +
                          ...
                          $"                    bat '\"C:\\\\Tools\\\\Octopus\\\\Octo.exe\" pack --id=\"{OctopusProjectName}.Debug\" --version=1.0.%BUILD_NUMBER%+%BRANCH_NAME% --basePath {projectName}/bin/Debug'\r\n" +
                          $"                    bat '\"C:\\\\Tools\\\\Octopus\\\\Octo.exe\" pack --id=\"{OctopusProjectName}.Production\" --version=1.0.%BUILD_NUMBER%+%BRANCH_NAME% --basePath {projectName}/bin/Release'\r\n" +
                          $"                    bat '\"C:\\\\Tools\\\\Octopus\\\\Octo.exe\" push --package {OctopusProjectName}.1.0.%BUILD_NUMBER%+%BRANCH_NAME%.nupkg --server %OctopusServer% --apiKey %OctopusApiKey% --replace-existing'\r\n" +
